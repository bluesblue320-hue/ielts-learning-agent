...
def _response(row: WritingPractice) -> PracticeResponse:
    return PracticeResponse(
        id=row.id, learner_id=row.learner_id, recommendation_id=row.recommendation_id,
        target_skill=row.target_skill, question=row.question,
        focus_objective=row.focus_objective, instructions=list(row.instructions),
        checkpoints=list(row.checkpoints), practice_type=row.practice_type,
        generator_policy_version=row.generator_policy_version, provider=row.provider,
        model=row.model, prompt_version=row.prompt_version,
        thinking_mode=row.thinking_mode,
        lifecycle_state=PracticeLifecycleState(row.lifecycle_state),
        attempt_id=row.attempt_id, created_at=row.created_at, updated_at=row.updated_at,
    )


@router.post("/recommendations/{recommendation_id}/practice", response_model=GenerationOutcome)
...
async def submit_practice(
    learner_id: int, practice_id: int, payload: PracticeSubmission,
