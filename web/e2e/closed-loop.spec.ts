...
test("Chinese learner completes one authoritative adaptive writing loop", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /综合分数/ })).toBeVisible();
  await page.reload();
  await expect(page.getByText("该练习已提交，正在恢复持久化评估。")).toBeVisible();
  await expect(page.getByRole("heading", { name: /综合分数/ })).toBeVisible();
  await expect(page.getByRole("button", { name: "提交作文并获取评估" })).toHaveCount(0);
  await page.getByRole("button", { name: "完成练习并获取下一步建议" }).click();
  await expect(page).toHaveURL(/\/dashboard$/);
  await expect(page.getByText("已采纳证据：2").first()).toBeVisible();
  await expect(page.getByText("训练重点：")).toBeVisible();
