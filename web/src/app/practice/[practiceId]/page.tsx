...
export default function PracticePage() {
  ...
  async function completePractice() {
    if (learnerId === null || evaluation === null) return;
    setIsCompleting(true); setError(null);
    try {
      const result = await apiClient.completePractice(learnerId, practiceId);
      setRecommendation(result.next_recommendation_id, result.next_recommendation);
      router.push("/dashboard");
    } catch { setError("暂时无法更新学习状态，请稍后重试。"); }
  ...
  if (practice === null) return <p className="status-copy" aria-live="polite">{error ?? "正在读取练习…"}</p>;
  ...
  return <section className="writing-page"><p className="eyebrow">针对性练习</p><h1>{practice.focus_objective}</h1><article className="content-card"><h2>Writing Task 2</h2><p className="question-copy">{practice.question}</p><h3>写作提示</h3><ul>{practice.instructions.map((item) => <li key={item}>{item}</li>)}</ul><h3>检查清单</h3><ul>{practice.checkpoints.map((item) => <li key={item}>{item}</li>)}</ul></article><section className="content-card next-step"><label htmlFor="practice-essay">你的英文作文</label><textarea disabled={submission !== null} id="practice-essay" value={essay} onChange={(event) => setEssay(event.target.value)} />{error !== null && <p className="error-message" role="alert">{error}</p>}{statusCopy !== null && <p className="status-copy" aria-live="polite">{statusCopy}</p>}{submission === null && <button className="primary-action" disabled={isSubmitting || essay.trim() === ""} onClick={submitPractice} type="button">{isSubmitting ? "正在提交…" : "提交作文并获取评估"}</button>}</section>{evaluation !== null && <><EvaluationDisplay evaluation={evaluation.evaluation} /><button className="primary-action" disabled={isCompleting} onClick={completePractice} type="button">{isCompleting ? "正在更新学习状态…" : "完成练习并获取下一步建议"}</button></>}</section>;
}
